...
test_that("KMEANS matches MATLAB benchmark", {
    ...
    result <- fsda_kmeans(Y, k = 1, replicates = 10, seed = 0)

    matlab_mu <- as.matrix(read.csv(
        system.file("extdata", "kmeans", "mu_MATLAB_kmeans_k1.csv", package = "fsdabridge"),
        header = FALSE
    ))
    storage.mode(matlab_mu) <- "double"

    matlab_obj <- as.numeric(read.csv(
        system.file("extdata", "kmeans", "obj_MATLAB_kmeans_k1.csv", package = "fsdabridge"),
        header = FALSE
    )[[1]])

    matlab_siz <- as.matrix(read.csv(
        system.file("extdata", "kmeans", "siz_MATLAB_kmeans_k1.csv", package = "fsdabridge"),
        header = FALSE
    ))
    storage.mode(matlab_siz) <- "double"

    matlab_idx <- as.integer(as.numeric(read.csv(
        system.file("extdata", "kmeans", "idx_MATLAB_kmeans_k1.csv", package = "fsdabridge"),
        header = FALSE
    )[[1]]))
    ...
    expect_lt(abs(result$obj - matlab_obj), 1e-9)
